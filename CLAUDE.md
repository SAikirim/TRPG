...
- **판정 결과는 반드시 공개** — 주사위를 굴린 후에는 TRPG답게 판정 과정과 결과를 보여준다
  - 주사위 표기: `🎲 1d20 → [15] + INT(+4) = 19 vs DC 16 → ✅ 성공!`
  - 데미지 표기: `⚔️ 1d8 → [6] + INT(+4) = 10 데미지`
  - 크리티컬/펌블 강조: `🎲 1d20 → [20] 💥 크리티컬!` / `🎲 1d20 → [1] 💀 펌블!`
- **나레이션 → 판정 → 결과 순서**: 먼저 상황을 묘사하고, 판정 결과를 보여주고, 결과에 따른 나레이션을 이어간다
