...
★ 터미널 출력 규칙 (show_system_log 설정):
  단계 헤더([1a], [1b], [2], [3])는 항상 표시한다 — 숨기지 않는다
  show_system_log가 제어하는 것은 각 단계의 "세부 내용"이다
  show_system_log: false (기본) → 단계 헤더만 표시, 세부 내용 숨김
    예: "[1a] GM 방향 설정 → [1b] 에이전트 호출 → [2] 나레이션 작성 → [3] 시스템 반영 → 나레이션"
  show_system_log: true → 단계 헤더 + 세부 내용(GM 판단, 에이전트 결과, 판정 상세) 모두 출력
```
