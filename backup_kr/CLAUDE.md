...
> 이 시스템은 두 계층이 있다: **플레이어** (실제 사람 / AI 페르소나)와 **캐릭터** (게임 내 존재).

| 계층 | 정체성 | 예시 | 대화 모드 |
|------|--------|------|----------|
| **플레이어 (OOC)** | 캐릭터를 조종하는 사람 | 준혁, 서연, 민지 | 게임 밖: 전략, 의견, 메타 토론 |
| **캐릭터 (IC)** | 게임 내 인물 | 사이키, 루세나, 노엘 | 게임 내: 대사, 행동, 롤플레이 |

### 매핑
- `agents/agent_a.json` (준혁) → `player_1` (사이키) 조종 — **유저 직접 조종**
- `agents/agent_b.json` (서연) → `player_2` (루세나) 조종 — **AI Player Agent**
- `agents/agent_c.json` (민지) → `player_3` (노엘) 조종 — **AI Player Agent**

### GM 판별 규칙
- **IC 입력**: 캐릭터 이름 사용, 게임 내 행동/대사 → GM 턴으로 처리 (나레이션)
- **OOC 입력**: 플레이어 이름 사용 (민지, 서연), 메타 발언 (전략, 규칙, 캐릭시트) → Player Agent를 OOC 모드로 호출, 플레이어 페르소나로 응답 (캐릭터가 아님)
- OOC 응답은 나레이션이나 이벤트 로그에 포함하지 않는다
- 플레이어들이 OOC로 전략을 상의한 후, IC로 전환하여 실제 턴을 진행할 수 있다
